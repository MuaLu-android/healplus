...
fun CheckOutScreen(navController: NavController,
    ...
    Scaffold (
        ...
        Column (
            ...
            PaymentSummary(totalAmount, tax,
                onClick = {
                    val selectedProductsJson = Gson().toJson(selectedProducts.toList())
                    Log.d("PaymentSummary", "Danh sách sản phẩm JSON: ${selectedProductsJson}")
                    addressItems.value.forEach{item ->
                        ...
                        try {
                            ...
                            apiCallOder.addOrder(item.fullName, item.phoneNumber, "item.email", userId,
                                Address, quantity.toString(), "700", "480",
...
@Composable
fun PaymentSummary(totalAmount: Double, tax: Double, onClick: () -> Unit) {
    Card(shape = RoundedCornerShape(8.dp), modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            Row (modifier = Modifier
                ...
                horizontalArrangement = Arrangement.SpaceBetween){
                Column {
                    Text("480.000đ", fontWeight = FontWeight.Bold, color = Color.Red)
