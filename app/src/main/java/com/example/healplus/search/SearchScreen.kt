package com.example.healplus.search
...
import com.example.healplus.category.RecommendedList1
...
fun SearchScreen(
    ...
    LaunchedEffect(search) {
        delay(500) // Chờ 500ms sau khi người dùng dừng nhập
        Log.d("SEARCH_EFFECT", "Search query: $search")
        if (search.isNotEmpty()) {
            viewModel.loadProductBySearch(search)
        }
    }
    Scaffold (
        topBar = { TopAppBarSearch(navController, search,
            onSearchChange = {it ->
                search = it
            })}
    ){ paddingValues ->
...
fun TopAppBarSearch(
    ...
    var search1 by remember { mutableStateOf(search) }
    TopAppBar(
        title = {
            TextField(
                value = search1,
